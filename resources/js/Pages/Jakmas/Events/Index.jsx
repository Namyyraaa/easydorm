...
export default function JakmasEventsIndex() {
  const { props } = usePage();
  ...
  useEffect(() => { setPage(1); }, [statusFilter, typeFilter, mineOnly]);
  ...
  // Embedded create form (styled like staff)
  const { data, setData, post, processing, errors } = useForm({
    name: '', description: '', visibility: 'open', type: 'event',
    starts_at: '', ends_at: '', registration_opens_at: '', registration_closes_at: '',
    capacity: '', dorm_id: '', images: [],
  });
  ...
  const submitCreate = (e) => {
    e.preventDefault();
    post(route('jakmas.events.store'), { forceFormData: true, preserveScroll: true });
  };
  ...
  useEffect(() => {
    ...
    if (activeTab === 'manage') url.searchParams.delete('tab');
    else url.searchParams.set('tab', 'create');
        ...
        <div className="mx-auto max-w-5xl sm:px-6 lg:px-8">
          <div className="border-b border-violet-200 mb-6">
            <nav className="-mb-px flex w-full" aria-label="Tabs">
              <button type="button" onClick={() => setActiveTab('create')} className={`whitespace-nowrap border-b-2 px-1 pb-2 text-sm font-medium flex-1 text-center order-2 ${activeTab === 'create' ? 'border-violet-500 text-violet-800' : 'border-transparent text-gray-500 hover:text-violet-700 hover:border-violet-300'}`}>Create Event</button>
              <button type="button" onClick={() => setActiveTab('manage')} className={`whitespace-nowrap border-b-2 px-1 pb-2 text-sm font-medium flex-1 text-center order-1 ${activeTab === 'manage' ? 'border-violet-500 text-violet-800' : 'border-transparent text-gray-500 hover:text-violet-700 hover:border-violet-300'}`}>Manage Events</button>
            </nav>
          </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex gap-3">
                  <div className="flex items-center">
                    ...
                    <select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)} className="ml-2 rounded border-gray-300 text-sm focus:outline-none transition-shadow hover:ring-2 hover:ring-purple-300 hover:ring-offset-1 focus:ring-2 focus:ring-purple-700 focus:ring-offset-1">
                      {statuses.map(s => (<option key={s} value={s}>{s === 'all' ? 'All' : humanize(s)}</option>))}
                    </select>
                  </div>
                  <div className="flex items-center">
                    <label className="text-sm text-gray-600">Type</label>
                    <select value={typeFilter} onChange={e=>setTypeFilter(e.target.value)} className="ml-2 rounded border-gray-300 text-sm focus:outline-none transition-shadow hover:ring-2 hover:ring-purple-300 hover:ring-offset-1 focus:ring-2 focus:ring-purple-700 focus:ring-offset-1">
                      {types.map(t => (<option key={t} value={t}>{t === 'all' ? 'All' : humanize(t)}</option>))}
                    </select>
                  </div>
                ...
                <tbody>
                  {displayItems.map(e => (
                    <tr key={e.id} className="border-t">
                      ...
                      <td className="w-28">
                        <Link href={route('jakmas.events.show', e.id)} className="inline-flex items-center px-3 py-1 rounded bg-indigo-600 text-white hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-400 focus:outline-none focus:ring-offset-1">
                          View
                        </Link>
                      </td>
                    </tr>
            ...
            <div className="overflow-hidden rounded-xl border border-violet-100 bg-white p-6 shadow-sm">
              ...
              <form onSubmit={submitCreate} className="space-y-4 text-violet-900">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-violet-800">Dorm ID (for closed)</label>
                    <input type="number" className="mt-1 w-full rounded border border-violet-200 p-2 focus:border-violet-500 focus:ring-violet-500" value={data.dorm_id} onChange={e=>setData('dorm_id', e.target.value)} />
                    {errors.dorm_id && <p className="text-sm text-red-600">{errors.dorm_id}</p>}
                  </div>
                </div>
                <div>
                ...
                <div className="mt-6 flex justify-end gap-2">
                  <Link href={route('jakmas.events.index')} className="inline-flex items-center rounded-lg bg-gray-200 px-4 py-2 text-gray-800 shadow-sm transition hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-300">Cancel</Link>
