...
export default function Show({ event }) {
  ...
  const setPassword = (e) => {
    e.preventDefault();
    passwordForm.post(route('jakmas.events.setPassword', event.id));
  };
  const uploadMedia = (e) => {
    e.preventDefault();
    mediaForm.post(route('jakmas.events.media.upload', event.id), { forceFormData: true });
  };

  return (
    <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Event (JAKMAS): {event.name}</h2>}>
      <div className="p-6">
