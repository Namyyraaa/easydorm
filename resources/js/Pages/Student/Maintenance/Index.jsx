            ...
            <div className="divide-y">
              {items.map((it) => (
                <div key={it.id} className="py-3 flex items-center justify-between">
                  ...
                  <Link href={route('student.maintenance.show', it.id)} className="text-indigo-600 hover:underline">View</Link>
                </div>
              ))}
              {items.length === 0 && <p className="text-sm text-gray-600">No maintenance requests yet.</p>}
            </div>
          </div>
        </div>

        {/* Roommate-visible requests (read-only) */}
        <div className="mx-auto max-w-5xl sm:px-6 lg:px-8 mt-6">
          <div className="bg-white shadow sm:rounded-lg p-6">
